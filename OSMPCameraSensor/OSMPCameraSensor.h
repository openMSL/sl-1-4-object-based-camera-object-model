...
/* Set log path */
#define PUBLIC_LOGGING (1u)
#define PRIVATE_LOG_PATH "C:\\temp\\OSI-CameraSim.log"
...
/* FMU Class */
class COSMPCameraSensor {
public:
    /* FMI2 Interface mapped to C++ */
    COSMPCameraSensor(fmi2String theinstanceName, fmi2Type thefmuType, fmi2String thefmuGUID, fmi2String thefmuResourceLocation, const fmi2CallbackFunctions* thefunctions, fmi2Boolean thevisible, fmi2Boolean theloggingOn);
    ~COSMPCameraSensor();
...
protected:
...
#endif
            private_log_file << "OSMPCameraSensor" << "::Global:FMI: " << buffer << endl;
    ...
    void internal_log(const char* category, const char* format, va_list arg)
    {
        ...
        if (!private_log_file.is_open())
            private_log_file.open(PRIVATE_LOG_PATH, ios::out | ios::app);
        if (private_log_file.is_open()) {
            private_log_file << "OSMPCameraSensor" << "::" << instanceName << "<" << ((void*)this) << ">:" << category << ": " << buffer << endl;
            private_log_file.flush();
        }
#endif
...
protected:
    ...
    bool get_fmi_sensor_data_in(osi3::SensorData &data); // for VTD v2.2 only
