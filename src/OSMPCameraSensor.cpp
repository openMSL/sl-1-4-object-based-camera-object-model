...
fmi2Status OSMPCameraSensor::DoCalc(fmi2Real current_communication_point, fmi2Real communication_step_size, fmi2Boolean no_set_fmu_state_prior_to_current_pointfmi_2_component)
{
    ...
    if (get_fmi_sensor_data_in(current_in))
    {
        ...
        NormalLog(
            "DEBUG", "Sensor mounting postion and orientation: x %.2f, y %.2f, z %.2f, yaw %.2f, pitch %.2f, roll %.2f", mpos_x, mpos_y, mpos_z, mpos_yaw, mpos_pitch, mpos_roll);
             ...
             &sens_sv_z](const osi3::TrafficLight& trafficlight) {
                ...
                else
                {
                    NormalLog("OSI",
                              ...
                              trafficlight.base().position().y(),
                              trafficlight.base().position().z());
                    // normal_log("OSI", "Ignoring stobjicle %d[%llu] Outside Sensor Scope Object trans_x,trans_y,trans_z, distance to sensor,winkel %f,%f,%f (%f,%f,%f) ,%f,%f", i,
                    // trafficlight.id().value(), trans_x, trans_y, trans_z, rel_x, rel_y, rel_z, distance,winkel);
                }
            });
